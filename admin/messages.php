...
error_log("DEBUG: admin/messages.php - Page loaded for user ID: " . $currentUserId);
...
try {
    ...
    error_log("DEBUG: admin/messages.php - Successfully marked messages as read for user ID: " . $currentUserId);
...
$contacts = [];
try {
    error_log("DEBUG: admin/messages.php - Preparing SELECT query for all users: SELECT id, name, role FROM users WHERE id != :currentUserId ORDER BY name ASC");
    $stmt = $pdo->prepare("SELECT id, name, role FROM users WHERE id != :currentUserId ORDER BY name ASC");
    $stmt->bindParam(':currentUserId', $currentUserId, PDO::PARAM_INT);
    $stmt->execute();
    $contacts = $stmt->fetchAll(PDO::FETCH_ASSOC);
    error_log("DEBUG: admin/messages.php - Fetched " . count($contacts) . " potential contacts (all other users).");
} catch (PDOException $e) {
...
if ($selectedContactId) {
    try {
        ...
        ";
        error_log("DEBUG: admin/messages.php - Preparing SELECT query for conversation messages: " . $sql);
        ...
        error_log("DEBUG: admin/messages.php - SELECT parameters for conversation messages: currentUserId=" . $currentUserId . ", selectedContactId=" . $selectedContactId);
        $stmt->execute();
        $conversationMessages = $stmt->fetchAll(PDO::FETCH_ASSOC);
        error_log("DEBUG: admin/messages.php - Number of conversation messages fetched: " . count($conversationMessages));
        ...
        $stmt->execute();
        error_log("DEBUG: admin/messages.php - Marked specific messages from " . $selectedContactName . " to " . $currentUserId . " as read (read_status=1, read_at=NOW()). Rows affected: " . $stmt->rowCount());
